...
folder_name = "London 2023"
group_file_name = "B_groups.csv"

# notes
# removed delegates from original groups files
# need to fill blanks with 0s - in code now
#######
...
for name, values in df_groups.iteritems():
    ...
    dict_stages[name][["Stage", "Group"]
                      ] = dict_stages[name]["Group"].str.split(" ", expand=True)
    dict_stages[name]["Required"] = STAGE_COUNT
    dict_stages[name]['Combined'] = dict_stages[name]['Stage'] + \
        " " + dict_stages[name]['Group']
...
for event in events:
    ...
    dict_maxPerStage = pd.to_numeric(df_stageInfo.groupby(
        ["Stage"])["Group"].max(), errors="ignore").to_dict()
    ...
    for index, row in df_groupsDelimit.iterrows():
        ...
        if row[0] == "0":  # if competitor is not in event, do not account
            ...
            if row[1] == dict_maxPerStage[row[0]]:
                ...
                try:
                    # if any other group, judge group after
                    dummy = row[0] + " " + str(int(row[1]+1))
                except:
                    dummy = row[0] + " " + str(row[1]+1)
    ...
    # sort by lowest to highest, to force new people to judge where required
    df_groups = df_groups.sort_values(by=["Count"], ascending="True")


df_judgeAssignmentPrint = df_judgeAssignment.fillna("")
df_judgeAssignmentPrint = df_judgeAssignmentPrint.drop("Count", axis=1)
try:
    df_judgeAssignmentPrint = df_judgeAssignmentPrint.sort_values(
        by=["Name"], ascending="True")
except:
    df_judgeAssignmentPrint = df_judgeAssignmentPrint.sort_values(
        by=["name"], ascending="True")

df_judgeAssignmentPrint.to_csv(
    "data/{}/judging_assignments_{}.csv".format(folder_name, comp_id), encoding='utf-8', index=True)
